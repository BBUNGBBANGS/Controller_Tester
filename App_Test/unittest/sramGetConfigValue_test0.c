#line 2 "cs_test%sramGetConfigValue_test0%cs_error"
/* sramGetConfigValue_test0.c */
#ifndef __sramGetConfigValue_test0__
#define __sramGetConfigValue_test0__

#include "../factory/factory.h"

#include "sramGetConfigValue_test0.h"

void CS_TEST sramGetConfigValue_test0()
{
	/*Do not modify CS_TEST_PARAMETER()*/
	CS_TEST_PARAMETER(sramGetConfigValue_test0);
	/*Do not remove connect_stub_configuration()*/
	int cs_stub_connect_function_call = connect_sramGetConfigValue_test0_stub_configuration();
	{/*<START TEST>*/


	/*Declaration (parameter/return/target object) variables*/
	struct tcramBase tcram1REG_tmp_mem [1]={0,}; 
	struct tcramBase tcram2REG_tmp_mem [1]={0,}; 
	sram_config_reg_t * config_reg  = 0x0;
	struct sram_config_reg config_reg_mem [1]={0,}; 
	enum config_value_type type ; 

	/*Memory assign*/
	tcram1REG_tmp = tcram1REG_tmp_mem;
	tcram2REG_tmp = tcram2REG_tmp_mem;
	config_reg = config_reg_mem;

	/*Input*/
	tcram1REG_tmp_mem[0].RAMCTRL = CS_UINT_INPUT(unsigned int,"tcram1REG_tmp_mem[0].RAMCTRL");
	tcram1REG_tmp_mem[0].RAMTHRESHOLD = CS_UINT_INPUT(unsigned int,"tcram1REG_tmp_mem[0].RAMTHRESHOLD");
	tcram1REG_tmp_mem[0].RAMINTCTRL = CS_UINT_INPUT(unsigned int,"tcram1REG_tmp_mem[0].RAMINTCTRL");
	tcram1REG_tmp_mem[0].RAMTEST = CS_UINT_INPUT(unsigned int,"tcram1REG_tmp_mem[0].RAMTEST");
	tcram1REG_tmp_mem[0].RAMADDRDECVECT = CS_UINT_INPUT(unsigned int,"tcram1REG_tmp_mem[0].RAMADDRDECVECT");
	tcram2REG_tmp_mem[0].RAMCTRL = CS_UINT_INPUT(unsigned int,"tcram2REG_tmp_mem[0].RAMCTRL");
	tcram2REG_tmp_mem[0].RAMTHRESHOLD = CS_UINT_INPUT(unsigned int,"tcram2REG_tmp_mem[0].RAMTHRESHOLD");
	tcram2REG_tmp_mem[0].RAMINTCTRL = CS_UINT_INPUT(unsigned int,"tcram2REG_tmp_mem[0].RAMINTCTRL");
	tcram2REG_tmp_mem[0].RAMTEST = CS_UINT_INPUT(unsigned int,"tcram2REG_tmp_mem[0].RAMTEST");
	tcram2REG_tmp_mem[0].RAMADDRDECVECT = CS_UINT_INPUT(unsigned int,"tcram2REG_tmp_mem[0].RAMADDRDECVECT");
	type = (enum config_value_type)CS_INT_INPUT(enum config_value_type,"type");



	/* sramGetConfigValue(struct sram_config_reg *, enum config_value_type) */
	sramGetConfigValue(config_reg,type);




	/*<END TEST>*/}
	/*<CS_DISCONNECT_CODE_START>*/
	/*Do not remove disConnect_stub_configuration()*/
	disConnect_sramGetConfigValue_test0_stub_configuration();
	/*<CS_DISCONNECT_CODE_END>*/
}

#endif
